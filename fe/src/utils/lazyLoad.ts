  ...
  try {
    ...
    for (const img of imgs) {
      try {
        ...
        if (inViewport) {
          try {
            logger.debug('[lazyLoad] force immediate load (visible after group change)', {
    ...
    try {
      if (typeof window !== 'undefined' && loaded.length > 0) {
        ...
        w.__LAZYLOAD_FORCED_COUNT = ((w.__LAZYLOAD_FORCED_COUNT as number) || 0) + loaded.length
        logger.debug('[lazyLoad] forced loads count', w.__LAZYLOAD_FORCED_COUNT)
      }
    } catch {
    ...
    try {
      logger.debug('[lazyLoad] ensureVisibleImagesLoad error', error)
    } catch {}
    return []
  }
}
...
export function observeLazyImages(
  ...
  try {
    ...
    try {
      const srcs = images
        .slice(0, 20)
        .map((i) => ({ src: i.getAttribute('src'), dataSrc: i.dataset.src }))
    ...
    if ('IntersectionObserver' in window) {
          ...
          (entries) => {
            entries.forEach((entry) => {
              if (entry.isIntersecting) {
                ...
                const ds = img.dataset.src
                try {
                  logger.debug('[lazyLoad] intersecting, loading', {
                    src: img.getAttribute('src'),
                    dataSrc: ds,
                    alt: img.alt,
                  })
      ...
      for (const img of images) {
        if (img.dataset.src) {
          try {
            ...
            if (inViewport) {
              try {
                logger.debug('[lazyLoad] immediate load (in-viewport)', {
                  src: img.getAttribute('src'),
            ...
            try {
              logger.debug('[lazyLoad] observe failed', e)
            } catch {}
          }
        } else {
          try {
            logger.debug('[lazyLoad] image has no data-src, skipping', {
              src: img.getAttribute('src'),
              alt: img.alt,
            })
          } catch {}
      ...
      for (const img of images) {
        if (img.dataset.src) {
          try {
            logger.debug('[lazyLoad] fallback loading image', {
              src: img.getAttribute('src'),
              dataSrc: img.dataset.src,
              alt: img.alt,
            })
